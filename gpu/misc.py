...
__version__ = '0.1'
__all__ = ['inplace_roll', 'pol_fill']


_INPLACE_ROLL = cp.RawKernel(r"""
extern "C" __global__
void inplace_roll(float2* signals,
                  int offset,
                  int nStand,
                  int nChan,
                  int nWin) {
...
def inplace_roll(signals, offset, blockDim=(4,16)):
...
void pol_fill(const float2 *signals,
  ...
  if( c >= nChan || w >= nWin) {
    ...
    int s;
    for(s=0; s<nStand; s+=2) {
      float2 tempA = *(signals + (s+0)*nChanAct*nWinAct + c*nWinAct + w);
      float2 tempB = *(signals + (s+1)*nChanAct*nWinAct + c*nWinAct + w);
      unsigned char validA = *(valid + (s+0)*nWinAct + w);
      unsigned char validB = *(valid + (s+1)*nWinAct + w);
      
      if( *(swap + s/2) ) {
        *(outputX + (s/2+offset)*nChan*nWin + c*nWin + w) = tempB;
        *(outputY + (s/2+offset)*nChan*nWin + c*nWin + w) = tempA;
        *(validX + (s/2+offset)*nWin + w) = validB;
        *(validY + (s/2+offset)*nWin + w) = validA;
      } else {
        *(outputX + (s/2+offset)*nChan*nWin + c*nWin + w) = tempA;
        *(outputY + (s/2+offset)*nChan*nWin + c*nWin + w) = tempB;
        *(validX + (s/2+offset)*nWin + w) = validA;
        *(validY + (s/2+offset)*nWin + w) = validB;
      }
    }
  }
}
...
def pol_fill(signals, valid, swap, offset, outputX, validX, outputY, validY, blockDim=(4,16)):
    ...
    nChan, nWin = outputX.shape[1], outputX.shape[2]
    
    signals = copy_using_cache(signals)
    valid = copy_using_cache(valid)
    swap =copy_using_cache(swap)
    outputX = copy_using_cache(outputX)
    validX = copy_using_cache(validX)
    outputY = copy_using_cache(outputY)
    validY = copy_using_cache(validY)
