...
def main(args):
    ...
    # Get the raw polycos
    with open(refSrc._polycos.filename, 'r') as fh:
    ...
    oFreq = np.fft.fftshift(oFreq)*pulsarDoppler
    ...
    subIntCount = [0.0 for i in range(nProfileBins)]
    subIntWeight = [0 for i in range(nProfileBins)]
    subIntReset = [False for i in range(nProfileBins)]
    fileCount   = [0 for i in range(nProfileBins)]
    avis = [None for i in range(nProfileBins)]
    wallStart = time.time()
    done = False
        ...
        for j in range(nSub):
            ...
            if nDRXInputs > 0:
                with streamD:
                    ...
                    dataDSubF[0::2,:] /= numpy.sqrt(dipoleX) * 7
                    dataDSubF[1::2,:] /= numpy.sqrt(dipoleY) * 7
