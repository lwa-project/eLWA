...
"""
Given a collection of .npz files search for course delays and rates using FFTs.
"""
...
def main(args):
    ...
    delay = np.fft.fftfreq(nchan, d=freq[1]-freq[0])
    delay = np.fft.fftshift(delay)
    dres = delay[1] - delay[0]
    
    drate = np.fft.fftfreq(nInt, d=tInt)
    drate = np.fft.fftshift(drate)
    rres = drate[1] - drate[0]
    
    print("Searching delays %.1f to %.1f us in steps of %.2f us" % (delay[0]*1e6, delay[-1]*1e6, dres*1e6))
    print("           rates %.1f to %.1f mHz in steps of %.2f mHz" % (drate[0]*1e3, drate[-1]*1e3, rres*1e3))
    print(" ")
