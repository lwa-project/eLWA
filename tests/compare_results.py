...
for hdu1,hdu2 in zip(hdulist1, hdulist2):
    ...
    for row1,row2 in zip(hdu1.data, hdu2.data):
        for f in range(len(row1)):
            try:
                same_value = numpy.allclose(row1[f], row2[f], atol=5.5e-7)
            except TypeError:
            ...
            if not same_value:
                ...
                try:
                    for i in range(len(row1[f])):
                        print("   - %i at %s vs %s -> %s and %s" % (i, row1[f][i], row2[f][i], numpy.abs(row1[f][i]-row2[f][i]), numpy.abs(row1[f][i]-row2[f][i])/row2[f][i]))
                except TypeError:
                    pass
            assert(same_value)
        r += 1
